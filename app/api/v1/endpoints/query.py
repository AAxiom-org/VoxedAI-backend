...
async def _stream_query(request: QueryRequest) -> StreamingResponse:
    ...
    async def generate():
        ...
        try:
            ...
            raw_context = []
            if request.use_rag:
                # Use the sanitized query to retrieve context
            ...
            # Now create sources with complete file content and metadata
            sources = []
            context = []  # Initialize context list here to ensure it's populated even if no sources are found
            ...
            for file_id, docs in grouped_context.items():
                ...
                # Also add to context list for the LLM
                context.append({
                    "text": full_text,
                    "score": source.score,
                    "file_id": file_id,
            ...
            sources_json = json.dumps({"type": "sources", "data": sources_dict})
            yield f"data: {sources_json}\n\n"
            
            # Log the context being sent to the LLM
            logger.info(f"DEBUG - Streaming answer with processed context: {len(context)} sources")
